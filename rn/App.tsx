...
export default function App() {
  ...
  const colors = useMemo(() => ({
    bg: dark ? '#121214' : '#FAF9FC',
    card: dark ? '#202024' : '#FFFFFF',
    text: dark ? '#F5F4F8' : '#1F1D23',
    muted: dark ? '#B8B5BE' : '#65616C',
    accent: dark ? '#D0BCFF' : '#6750A4',
    bubble: dark ? '#4C3A70' : '#E8DEFF',
  }), [dark]);
  ...
  const chooseModel = async () => {
    try {
      ...
      const copied = file.fileCopyUri;
      if (!copied) throw new Error('Android could not create a local readable copy of the GGUF file.');
      const target = `${RNFS.CachesDirectoryPath}/${file.name || 'model.gguf'}`;
      if (copied.replace('file://', '') !== target) {
        if (await RNFS.exists(target)) await RNFS.unlink(target);
        await RNFS.copyFile(copied, target);
      }
      setModelPath(`file://${target}`);
      setModelName(file.name || 'Selected GGUF model');
      setStatus(`${((file.size || 0) / 1024 / 1024 / 1024).toFixed(2)} GB copied locally and ready to load`);
    } catch (error: any) {
      if (!DocumentPicker.isCancel(error)) Alert.alert('Model selection failed', error?.message || String(error));
    }
  };

  const loadModel = async () => {
    if (!modelPath) return chooseModel();
    setLoading(true);
    ...
    try {
      ...
      Alert.alert('PocketMind', error?.message || String(error));
    } finally {
      setLoading(false);
    }
  };
  ...
  const send = async () => {
    ...
    try {
      ...
      const result = await context.current.completion({
        messages: [
          {role: 'system', content: 'You are a direct helpful assistant. Use clean Markdown. Put complete code, HTML, SVG, JSON, tables, or documents in fenced code blocks so the app can show them as artifacts.'},
          ...history,
        ],
        n_predict: 512,
        temperature: 0.65,
  ...
  const stop = async () => {
    try { await context.current?.stopCompletion(); } finally { setGenerating(false); }
  };
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.root}>
        <View style={styles.header}>
          ...
          <Button title="New" onPress={() => setMessages([])} color={colors.accent}/>
        </View>

        <View style={styles.modelCard}>
          <View style={styles.modelRow}>
            <View style={{flex: 1}}><Text style={styles.modelName}>{modelName}</Text><Text style={styles.muted}>{status}</Text></View>
            <Button title="Choose" onPress={chooseModel} color={colors.accent}/>
          </View>
          <TouchableOpacity style={styles.loadButton} onPress={loadModel} disabled={loading}>
            {loading ? <ActivityIndicator color="#fff"/> : <Text style={styles.loadText}>{context.current ? 'Reload model' : 'Load model'}</Text>}
          </TouchableOpacity>
        </View>
        ...
        <ScrollView style={styles.chat} contentContainerStyle={{paddingVertical: 16}}>
          ...
          {messages.map((message, index) => {
            ...
            return <View key={index} style={message.role === 'user' ? styles.userBubble : styles.assistantBubble}>
              ...
              {artifacts.map((item, artifactIndex) => <TouchableOpacity key={artifactIndex} style={styles.artifactButton} onPress={() => setArtifact(item)}><Text style={styles.artifactText}>Open {item.type || 'artifact'}</Text></TouchableOpacity>)}
            </View>;
          })}
        </ScrollView>

        <View style={styles.composer}>
          <TextInput style={styles.input} placeholder="Message your local model" placeholderTextColor={colors.muted} value={input} onChangeText={setInput} multiline editable={!generating}/>
          <Button title={generating ? 'Stop' : 'Send'} onPress={generating ? stop : send} color={colors.accent}/>
        </View>
      </View>

      {artifact && <View style={styles.overlay}>

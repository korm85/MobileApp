...
import { DEFAULT_SESSION_ID, MODELS } from './src/constants';
...
import { downloadModel, getModelPath, modelExists } from './src/services/ModelService';
import { initializeDatabase, loadArtifacts, loadMessages, saveArtifact, saveMessage } from './src/services/StorageService';
import { darkTheme, lightTheme } from './src/theme';
import { AppTheme, Artifact, Message, ModelState } from './src/types';
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const [savedMessages, savedArtifacts] = await Promise.all([loadMessages(DEFAULT_SESSION_ID), loadArtifacts()]);
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const response = await LlamaService.getInstance().generateResponse(prompt, stream.append, history);
  ...
  const handleDownload = async (modelId: string) => {
    ...
    try {
      await downloadModel(modelId, (progress) => setModelState((current) => ({ ...current, [modelId]: { status: 'downloading', progress } })));
      setModelState((current) => ({ ...current, [modelId]: { status: 'ready', progress: 1 } }));
    } catch (error) {
  ...
  const handleLoad = async (modelId: string) => {
    ...
    try {
      await LlamaService.getInstance().initialize(getModelPath(model.filename), { contextSize: 2048, threads: 4, gpuLayers: 0 });
...
function MessageBubble({ message, theme, artifacts, onOpenArtifact }: { message: Message; theme: AppTheme; artifacts: Artifact[]; onOpenArtifact: (artifact: Artifact) => void }) {
  ...
  const readable = isUser ? message.content : message.content.replace(/```(?:html|HTML)[\s\S]*?```/g, '').replace(/^#{1,6}\s*/gm, '').replace(/\*\*(.*?)\*\*/g, '$1').trim();
...
const styles = StyleSheet.create({
  ...
  settingRow: { paddingVertical: 16, borderBottomWidth: StyleSheet.hairlineWidth }, empty: { alignItems: 'center', paddingHorizontal: 20, marginTop: 90 }, emptyIcon: { fontSize: 42, marginBottom: 14 }, muted: { fontSize: 14 },
});
